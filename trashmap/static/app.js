window.onload = init();
var map_container, dumpsters, dumpsters_clustered;
function init() {
    // Setup Handlebars
    setup_handlebars();
    ...
    dumpsters_good = new DumpsterLayer('api/dumpster/tiles/{z}/{x}/{y}/good');
    dumpsters_neutral = new DumpsterLayer('api/dumpster/tiles/{z}/{x}/{y}/neutral');
    dumpsters_bad = new DumpsterLayer('api/dumpster/tiles/{z}/{x}/{y}/bad');
    //dumpsters_clustered = new ClusteredDumpsterLayer('api/dumpster/tiles/clustered/{z}/{x}/{y}');
    dumpsters_good.addToMap(map_container.map, 0, 18);
    dumpsters_neutral.addToMap(map_container.map, 13, 18);
    dumpsters_bad.addToMap(map_container.map, 13, 18);
...
var templates = {
    ...
    'marker_popup': Handlebars.compile($("#dumpster_popup_template").html()),
    'marker_popup_comments': Handlebars.compile($("#dumpster_popup_comments_template").html()),
    'feature_modal_body': Handlebars.compile($("#feature_modal_body").html())
};
